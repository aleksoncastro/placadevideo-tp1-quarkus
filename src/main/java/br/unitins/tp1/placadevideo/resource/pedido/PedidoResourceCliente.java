...
public class PedidoResourceCliente {
    ...
    public Response findItensPedidoByIdPedido(@PathParam ("id") Long idPedido){
        String username = jwt.getSubject();
        LOG.infof("Buscando itensPedido para o usuário: %s", username);
        return Response
                .ok(pedidoService.findByPedidoId(idPedido).stream().map(o -> ItemPedidoResponsetDTO.valueOf(o)).toList())
                .build();
    }
